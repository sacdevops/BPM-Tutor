"""Seed the database with initial task and admin data on first run."""
from __future__ import annotations

# ---------------------------------------------------------------------------
# Canonical seed data — task definitions (EN + DE)
# Kept here so config.py stays purely environment-config and does not mix in
# application data that belongs to the DB layer.
# ---------------------------------------------------------------------------
_SEED_TASKS = [
    {
        "id": "task_01",
...
def seed_tasks_from_config() -> int:
    """
    Import the canonical seed tasks into the Task table if they don't exist yet.
...
def seed_default_agent() -> bool:
    """Seed all 5 built-in system agents if they don't exist yet. Returns True if any were created."""
    return seed_default_agents()


def seed_default_agents() -> bool:
    """
    Create the five built-in system AI agents (Mentor, Assistant, Colleague, Supervisor, Delegant)
    if they don't already exist. Returns True if at least one agent was created.
    """
    try:
        from app.extensions import db
        ...
        _AGENTS = [
            ...
            },
            {
                ...
                'description': (
                    ...
                    'Agent decides when the model meets requirements.'
                ),
                'is_default': False,
                'sort_order': 4,
        ...
        created = False
        for spec in _AGENTS:
            if AIAgent.query.filter_by(agent_type=spec['agent_type'], is_system=True).first():
                continue
            ...
            agent = AIAgent(
                ...
                prompt_greeting_en=get_system_prompt(atype, 'greeting', 'en'),
                prompt_greeting_de=get_system_prompt(atype, 'greeting', 'de'),
                prompt_analysis_en=get_system_prompt(atype, 'analysis', 'en'),
                prompt_analysis_de=get_system_prompt(atype, 'analysis', 'de'),
                prompt_reaction_en=get_system_prompt(atype, 'reaction', 'en'),
                prompt_reaction_de=get_system_prompt(atype, 'reaction', 'de'),
            )
        ...
        try:
            ...
            current_app.logger.warning('[agents] seed_default_agents failed: %s', exc)
        except Exception:
            pass
        return False
...
def reset_system_agent_prompts() -> int:
    """
    Overwrite the stored prompts of all system agents with the current
    built-in prompt definitions.  Returns the number of agents updated.

    Safe to call at any time; skips agents with no matching agent_type.
    """
    try:
        from app.extensions import db
        from app.models.agent import AIAgent
        ...
        for agent in AIAgent.query.filter_by(is_system=True).all():
            ...
            for pt in ('greeting', 'analysis', 'reaction'):
                for lang in ('en', 'de'):
                    new_val = get_system_prompt(atype, pt, lang)
                    if new_val:
                        setattr(agent, f'prompt_{pt}_{lang}', new_val)
            count += 1
